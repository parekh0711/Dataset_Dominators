...
class MozillaCommonVoiceDataset:
    ...
    def _get_common_voice_filenames(self, dataframe_name='Voice_Samples_Training_Path.txt'):
        mozilla_metadata = pd.read_csv(os.path.join(self.basepath, dataframe_name))
        clean_files = mozilla_metadata['path'].values
        # print(clean_files)
        # input()
        # clean_files = [os.path.join(self.basepath, 'TrainingAudio/' + c) for c in clean_files]
        np.random.shuffle(clean_files)
        print("Total number of training examples:", len(clean_files))
        return clean_files

    def get_train_val_filenames(self):
        clean_files = self._get_common_voice_filenames(dataframe_name='Voice_Samples_Training_Path.txt')

        # resolve full path
        clean_files = [os.path.join(self.basepath, 'TrainingAudio', filename) for filename in clean_files]
    ...
    def get_test_filenames(self):
        clean_files = self._get_common_voice_filenames(dataframe_name='TestingAudio_Path.txt')

        # resolve full path
        clean_files = [os.path.join(self.basepath, 'TestingAudio', filename) for filename in clean_files]
